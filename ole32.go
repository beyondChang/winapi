...
func init() {
	...
	createStreamOnHGlobal = MustGetProcAddress(libole32, "CreateStreamOnHGlobal")
	...
	if hr := OleInitialize(); FAILED(hr) {
		panic("OleInitialize Error: " + syscall.E2BIG.Error())
	}
}
...
func CreateStreamOnHGlobal(hGlobal HGLOBAL, fDeleteOnRelease BOOL, ppstm *uintptr) HRESULT {
	ret, _, _ := syscall.Syscall(createStreamOnHGlobal, 3,
		uintptr(hGlobal),
