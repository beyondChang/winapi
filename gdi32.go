...
func init() {
	...
	createPen = MustGetProcAddress(libgdi32, "CreatePen")
	...
	setTextCharacterExtra = MustGetProcAddress(libgdi32, "SetTextCharacterExtra")
...
func CreatePen(fnPenStyle int32, nWidth int32, crColor COLORREF) HPEN {
	ret, _, _ := syscall.Syscall(createPen, 3,
		uintptr(fnPenStyle),
		uintptr(nWidth),
		uintptr(crColor))
...
func SetTextCharacterExtra(hdc HDC, nCharExtra int32) int32 {
	ret, _, _ := syscall.Syscall(setTextCharacterExtra, 2,
		uintptr(hdc),
		uintptr(nCharExtra),
		0)
...
// TextOut 封装了 Windows API 的 TextOutW 函数，用于在指定位置输出文本。
// 参数：
//
//	hdc - 设备环境句柄
//	x, y - 输出文本的起始坐标
//	lpString - 指向要输出的文本（UTF-16）
//	c - 字符数
//
// 返回值：true 表示成功，false 表示失败
func renderText(hdc HDC, x, y int32, lpString *uint16, c int32) bool {
...
// 设置字体并输出文本
func RenderText(hdc HDC, text string, x, y, fontSize int, bold bool, fontName string) {
	// 构造 LOGFONT
	var lf LOGFONT
	lf.LfHeight = int32(-fontSize) // 负值表示字符高度
	lf.LfWeight = 400
	if bold {
		lf.LfWeight = 700 // 粗体
	}
	...
	// 针对低分辨率打印机的优化设置
	lf.LfQuality = CLEARTYPE_QUALITY  // 使用 ClearType 渲染，比 ANTIALIASED_QUALITY 更清晰
	lf.LfOutPrecision = OUT_TT_PRECIS // TrueType 输出精度
	lf.LfClipPrecision = CLIP_DEFAULT_PRECIS
	lf.LfCharSet = GB2312_CHARSET // 中文字符集

	// 设置字体名称
	nameRunes := []rune(fontName)
	for i := 0; i < len(nameRunes) && i < len(lf.LfFaceName); i++ {
	...
	// 设置文本渲染模式 - 提高低分辨率下的清晰度
	SetTextCharacterExtra(hdc, 5) // 字符间距

	// 输出文本
	textPtr, _ := syscall.UTF16PtrFromString(text)
	renderText(hdc, int32(x), int32(y), textPtr, int32(len([]rune(text))))
...
func RenderTextWithSpacing(hdc HDC, text string, x, y, fontSize int, bold bool, fontName string, charExtra int) {
	// 构造 LOGFONT
	var lf LOGFONT
	lf.LfHeight = int32(-fontSize) // 负值表示字符高度
	lf.LfWeight = 400
	...
	// 设置字符间距
	SetTextCharacterExtra(hdc, int32(charExtra))
...
func RenderImage(hdc HDC, img image.Image, x, y, width, height int) {
	buf, w, h := ImageToBGR24(img)
	var bmi BITMAPINFO
	...
	hBitmap := CreateDIBSection(memDC, &bmi, DIB_RGB_COLORS, &bitsPtr, 0, 0)
	SelectObject(memDC, HGDIOBJ(hBitmap))
	copy((*[1 << 30]byte)(bitsPtr)[:len(buf):len(buf)], buf)
	StretchBlt(hdc, int32(x), int32(y), int32(width), int32(height),
		memDC, 0, 0, int32(w), int32(h), SRCCOPY)
	DeleteObject(HGDIOBJ(hBitmap))
	DeleteDC(memDC)
}

func RenderQRCode(hdc HDC, text string, x, y, width, height int) error {
	// 生成更高分辨率的二维码图片，减少缩放时的锯齿
	img, err := GenerateQRCodeImage(text, 2000) // 增加生成分辨率
	if err != nil {
		return fmt.Errorf("二维码生成失败：%v", err)
	}
	RenderImageOptimized(hdc, img, x, y, width, height)
	return nil
}

// RenderImageOptimized 针对低分辨率打印机优化的图片渲染
func RenderImageOptimized(hdc HDC, img image.Image, x, y, width, height int) {
	buf, w, h := ImageToBGR24(img)
	var bmi BITMAPINFO
	bmi.BmiHeader.BiSize = uint32(unsafe.Sizeof(BITMAPINFOHEADER{}))
	bmi.BmiHeader.BiWidth = int32(w)
...
// 生成二维码图片并返回 image.Image
func GenerateQRCodeImage(data string, size int) (image.Image, error) {
	// 使用最高质量设置生成二维码
	pngData, err := qrcode.Encode(data, qrcode.Highest, size)
	if err != nil {
		return nil, err
	}
...
// 将 image.Image 转为 24位RGB 像素数据（BGR顺序，GDI要求）
func ImageToBGR24(img image.Image) ([]byte, int, int) {
	bounds := img.Bounds()
	w, h := bounds.Dx(), bounds.Dy()
	// GDI DIBSection 要求每行4字节对齐
	rowSize := ((w*3 + 3) / 4) * 4
	...
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			...
			// 混合白色背景 (处理透明度)
			// RGBA() 返回的是预乘 Alpha 的值 (0-0xFFFF)
			// 混合公式: Final = Source + Dest * (1 - Alpha)
			// Dest 是白色 (0xFFFF)
			// 因为 r, g, b 已经是 Source * Alpha，所以只需要加背景部分
			invA := 0xFFFF - a
			r = r + invA
			g = g + invA
			b = b + invA

			offset := (h-1-y)*rowSize + x*3 // GDI位图从下到上
...
// DrawHorizontalLine 绘制横线
// hdc: 设备环境句柄
// x1, y: 起始坐标
// x2: 结束X坐标
// thickness: 线条粗细（像素）
// color: 线条颜色（RGB格式，如0x000000为黑色）
func DrawHorizontalLine(hdc HDC, x1, y, x2 int, thickness int) {
	var color COLORREF
	color = 0x000000 // 默认黑色
	// 自定义粗细的画笔
	pen := CreatePen(PS_SOLID, int32(thickness), color)
	...
	// 绘制横线
	MoveToEx(hdc, x1, y, nil)
	LineTo(hdc, int32(x2), int32(y))
...
// DrawVerticalLine 绘制竖线
// hdc: 设备环境句柄
// x, y1: 起始坐标
// y2: 结束Y坐标
// thickness: 线条粗细（像素）
// color: 线条颜色（RGB格式，如0x000000为黑色）
func DrawVerticalLine(hdc HDC, x, y1, y2 int, thickness int) {
	var color COLORREF
	color = 0x000000 // 默认黑色
	// 自定义粗细的画笔
	pen := CreatePen(PS_SOLID, int32(thickness), color)
	...
	// 绘制竖线
	MoveToEx(hdc, x, y1, nil)
	LineTo(hdc, int32(x), int32(y2))
...
// thickness: 线条粗细（像素）
// color: 线条颜色（RGB格式）
func DrawRectangle(hdc HDC, x1, y1, x2, y2 int, thickness int, color COLORREF) {
	// 创建画笔
	var pen HPEN
	if thickness == 1 {
	...
	// 绘制矩形
	Rectangle_(hdc, int32(x1), int32(y1), int32(x2), int32(y2))
...
// thickness: 线条粗细（像素）
// color: 线条颜色（RGB格式）
func DrawSquare(hdc HDC, x, y, size int, thickness int) {
	DrawRectangle(hdc, x, y, x+size, y+size, thickness, 0x000000)
}

// RenderTextMultiline 在指定位置渲染多行文本，支持配置最大行数和每行最大字数
// 参数：
//   - maxLines: 最大行数，0表示无限制
//   - maxCharsPerLine: 每行最大字数，0表示无限制
func RenderTextMultiline(hdc HDC, text string, x, y, fontSize, maxLines, maxCharsPerLine int, bold bool, fontName string) {
	// 如果文本为空，则直接返回
	if text == "" {
		return
	}

	// 如果没有设置每行最大字数，则使用一个合理的默认值
	if maxCharsPerLine <= 0 {
		maxCharsPerLine = 50 // 默认每行最多50个字符
	}
	...
	for _, r := range runes {
		...
		// 检查每行最大字数限制（主要控制逻辑）
		if len([]rune(testLine)) > maxCharsPerLine {
			// 当前行已达到最大字数限制
			if currentLine != "" {
				// 检查是否超过最大行数
				if maxLines > 0 && lineCount >= maxLines-1 {
					// 最后一行添加省略号
					if len([]rune(currentLine)) > 3 {
						truncatedLine := string([]rune(currentLine)[:len([]rune(currentLine))-3]) + "..."
						outputPtr, _ := syscall.UTF16PtrFromString(truncatedLine)
						renderText(hdc, int32(x), int32(currentY), outputPtr, int32(len([]rune(truncatedLine))))
					} else {
						outputLine := currentLine + "..."
						outputPtr, _ := syscall.UTF16PtrFromString(outputLine)
						renderText(hdc, int32(x), int32(currentY), outputPtr, int32(len([]rune(outputLine))))
					}
					lineCount++
					break
				}
